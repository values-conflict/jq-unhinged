...
def _gz_clord: [ 16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15, empty ];
	...
	def _gz_drain:
		...
		def _decode($tbl):
			...
			if .n < 1 then
				...
				$tbl[.v % 32768] as $e
				| if $e == null then
					error("gzip: invalid Huffman code (ph=\(.ph))")
				elif .n < $e.len then
					.sym = null
				else
					.sym = $e.sym
					| .v = (.v / $p2[$e.len] | floor) | .n -= $e.len
				end
			end
		;
		def _step:
			...
			| if $ph == "h0" then # ID1 = 0x1f
				if .n < 8 then
					...
					(.v % 256) as $b
					| if $b != 31 then
						error("gzip: bad ID1 (got \($b))")
					else
						.v = (.v / 256 | floor) | .n -= 8 | .ph = "h1" | _step
					end
				end
				...
				if .n < 8 then
					...
					(.v % 256) as $b
					| if $b != 139 then
						error("gzip: bad ID2 (got \($b))")
					else
						.v = (.v / 256 | floor) | .n -= 8 | .ph = "h2" | _step
					end
				end
				...
				if .n < 8 then
					...
					(.v % 256) as $b
					| if $b != 8 then
						error("gzip: unsupported CM \($b)")
					else
						.v = (.v / 256 | floor) | .n -= 8 | .ph = "h3" | _step
					end
				end
			elif $ph == "h3" then # FLG
				if .n < 8 then
				...
				if .n < 8 then
					...
					.rem = (.v % 256)
					| .v = (.v / 256 | floor) | .n -= 8 | .ph = "hx1" | _step
				end
				...
				if .n < 8 then
					...
					(.v % 256) as $b
					| .v = (.v / 256 | floor)
					| .n -= 8 | if $b == 0 then _next_hdr | _step else _step end
				end
				...
				if .n < 8 then
					...
					.v = (.v / 256 | floor) | .n -= 8 | .ph = "hh1" | _step
				end
			elif $ph == "hh1" then
				if .n < 8 then
					.
				else
					.v = (.v / 256 | floor) | .n -= 8 | _next_hdr | _step
				end
			...
			## ── DEFLATE BLOCK HEADER (RFC 1951 §3.2.3) ────────────────────────────
				...
				if .n < 3 then
					...
					| if $btype == 0 then
						...
						.ll = $fll | .dd = $fdd | .ph = "fl" | _step
					elif $btype == 2 then
						.ph = "dy0" | _step
					else
						error("gzip: reserved DEFLATE block type")
					end
				end

			## ── STORED BLOCK (BTYPE=00, RFC 1951 §3.2.4) ──────────────────────────
			elif $ph == "sa" then # align to byte boundary
				_align | .ph = "sl0" | _step
				...
				if .n < 8 then
					...
					.rem = (.v % 256)
					| .v = (.v / 256 | floor) | .n -= 8 | .ph = "sl1" | _step
				end
				...
				if .n < 8 then
					...
					.rem += (.v % 256) * 256
					| .v = (.v / 256 | floor) | .n -= 8 | .ph = "sn0" | _step
				end
			elif $ph == "sn0" then # NLEN byte 0 (discard; validity check omitted)
				if .n < 8 then
					.
				else
					.v = (.v / 256 | floor) | .n -= 8 | .ph = "sn1" | _step
				end
			elif $ph == "sn1" then # NLEN byte 1 (discard)
				if .n < 8 then
					.
				else
					.v = (.v / 256 | floor) | .n -= 8 | .ph = "sc" | _step
				end
			elif $ph == "sc" then # copy rem literal bytes from input stream
				if .rem == 0 then
					(if .bf == 1 then .ph = "gf" else .ph = "db" end) | _step
				elif .n < 8 then
					.
				else
					(.v % 256) as $b
					| .v = (.v / 256 | floor)
					| .n -= 8 | .rem -= 1 | _emit($b) | _step
				end
			...
			## ── FIXED HUFFMAN BLOCK (BTYPE=01, RFC 1951 §3.2.6) ───────────────────
				...
				((.w | length) - .bd) as $src
				| if $src < 0 then
					error("gzip: back-reference distance \(.bd) exceeds window \(.w | length)")
				else
			...
			## ── DYNAMIC HUFFMAN BLOCK (BTYPE=10, RFC 1951 §3.2.7) ─────────────────
				...
				if .n < 14 then
					...
					| .lnum = .hlit + .hdist
					| .lbuf = [ range(19) | 0 ]
					| .lpos = 0 | .rem = $hc + 4 | .ph = "dy1" | _step
				end
			elif $ph == "dy1" then # HCLEN+4 code-length alphabet entries (3 bits each)
				if .rem == 0 then
					.ll = _gz_htable(.lbuf)
					| .lbuf = [] | .prev = 0 | .ph = "dy2" | _step
				elif .n < 3 then
					.
				else
					.lbuf[$clord[.lpos]] = (.v % 8)
					| .v = (.v / 8 | floor)
					| .n -= 3 | .lpos += 1 | .rem -= 1 | _step
				end
			elif $ph == "dy2" then
				if (.lbuf | length) >= .lnum then
					...
					| if .sym == null then
						...
						error("gzip: invalid CL symbol \(.sym)")
					end
				end
				...
				if .n < 2 then
					...
					((.v % 4) + 3) as $c
					| .v = (.v / 4 | floor) | .n -= 2 | .prev as $p
					| .lbuf += [ range($c) | $p ] | .ph = "dy2" | _step
				end
				...
				if .n < 3 then
					...
					((.v % 8) + 3) as $c
					| .v = (.v / 8 | floor)
					| .n -= 3 | .lbuf += [ range($c) | 0 ] | .ph = "dy2" | _step
				end
				...
				if .n < 7 then
					...
					((.v % 128) + 11) as $c
					| .v = (.v / 128 | floor)
					| .n -= 7 | .lbuf += [ range($c) | 0 ] | .ph = "dy2" | _step
				end
			...
			# CRC32 and ISIZE are not verified; the 8 footer bytes are consumed and discarded.
				...
				. # "end" or unknown phase: drain is complete
			end
		;
		_step
	;
